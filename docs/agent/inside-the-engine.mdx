...
:::note[Scope: the in-process engine only]
This page describes the **in-process built-in engine** — the loop you get when the `[agent] model` ref is any plain `provider/model` (`qwen`, `moonshot`, `openai`, `anthropic`, `google`, `deepseek`). A ref of `claude-code/<model>` is different: it shells out to Anthropic's `claude` CLI as a subprocess and delegates its loop to *that* CLI, so none of the rules below govern it. See [Models](/en/agent/models/) for the two paths.
:::
...
This is a **native** tool-call loop — the structure rides on the provider's real `tools=[…]` API and the model's real `tool_calls`, not on the model hand-writing JSON into prose for the engine to parse. That distinction is the spine of the whole design. If you know OpenClaw, this is the familiar half: *a native tool-call loop, normalized at the provider boundary*, exactly OpenClaw's playbook — just pointed at a physical phone instead of a screen it lives on.

"Normalized at the provider boundary" is doing real work here. The six in-process vendors each return a slightly different reply envelope; the engine maps every one of them to a single internal shape the moment it comes in, so the loop logic above never has to know which provider it's talking to. It's provider-agnostic by the time it sees the model's choices.
...
The "one other tool" doesn't have to touch hardware. The agent has two kinds of tools: the [twelve MCP tools](/en/reference/mcp-tools/) (`peek`, `tap`, `swipe`, …) that get dispatched out to the rig, and a set of **local engine tools** — `note`, plan and scratchpad edits, memory and skill loads, `create_job`, `end_session` — that only do bookkeeping and never move the stylus. The note is always a local tool; the "other" slot can be a phone gesture *or* a local tool. That's why turns like `[note("scheduling a re-check in 30 min"), create_job(…)]` and `[note("verified, closing out"), end_session("DONE", …)]` are perfectly legal — the agent is taking one bookkeeping action that turn instead of one physical one.
