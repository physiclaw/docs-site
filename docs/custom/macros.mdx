...
Rehearse one gesture at a time with `--start-at` and `--stop-after` naming the same step, look at the phone, fix the step, and run it again. A pack's macro is addressed as `<app>/<name>`, an inline body as `<app>/<playbook>.<move>`. The studio's Macro tab does the same with a cursor over the steps.
