...
Verbs: `tap`, `double_tap`, `long_press` (a label, with `at`), `swipe` (`up`/`down`/`left`/`right`, with `at` and optional `size`/`speed`), `send_to_clipboard` (text), `wait` (seconds), and the bare words `home_screen`, `go_back`, `force_quit`, `peek`. Beside a verb: `at`, `when` or `skip_when` (run the step only while, or skip it while, a check holds; a screen that cannot be read skips a `when` step and runs a `skip_when` one), `require` and `forbid` (checked before the step), `expect` and `hint` (on a `wait`). A check is a text, a list of alternates, `{text, within}` with a band (`top`, `bottom`, `left`, `right`) or a box, or `{and|or|not}`. A pack macro may jump forward: `if_page: <name>` with `goto: <mark>` skips to `mark: <mark>` while the pack's page already shows, so the steps that reach it are not replayed; walked to, the mark checks the page arrived. Jumps follow one another, never one inside another.
