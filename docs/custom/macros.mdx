...
Available tools: `tap`, `double_tap`, `long_press`, `swipe`, `home_screen`, `go_back`, `force_quit`, `send_to_clipboard`, `peek`, and `wait`. The full reference for `guard`, `expect`, `skip_when` and the rest lives at `~/.physiclaw/macros/README.md`.
