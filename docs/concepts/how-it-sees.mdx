...
<CardGrid>
  ...
  <Card title="Icons → ONNX detector" icon="puzzle">
    **OmniParser V2** icon detection — a YOLO11m model finetuned by Microsoft, exported to ONNX
    (again no PyTorch at runtime). It finds interactable graphics — app icons, image buttons,
    toggles — that carry no text for OCR to read.
  </Card>
</CardGrid>

Both run fully **on-device**. No frame ever leaves your machine to be analysed, which is why
[install](/en/set-up/install/) fetches the model up front: it downloads
`microsoft/OmniParser-v2.0`'s `icon_detect/model.pt`, converts it to ONNX at `imgsz=1280`, and
leaves it at `~/.physiclaw/models/omniparser_icon_detect/model.onnx`. The two outputs are then
merged, cleaned (tiny boxes and low-confidence hits dropped, near-duplicates removed by overlap),
sorted top-to-bottom then left-to-right, and renumbered `0, 1, 2, …` — fresh every frame, so an
`id` is only a handle for *this* listing, never stable across looks.
...
| Field | What it is |
| --- | --- |
| `id` | a handle for that element *in this listing only* — renumbered every frame, never stable across looks. |
...
When the brain wants to act, it names a bbox and a [gesture](/en/reference/gestures/) —
`tap [0.51, 0.55, 0.59, 0.63]`. The server takes the **center** of that box,
`((left+right)/2, (top+bottom)/2)`, and aims there. So a loose box is fine as long as its middle
sits on the target; you're pointing, not tracing.
...
Because a screenshot can shift app state, the rule (echoed in the
[MCP tools reference](/en/reference/mcp-tools/)) is always to `peek` again after a `screenshot`
